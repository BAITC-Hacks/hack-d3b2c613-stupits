...
- **Карта пяти районов.** Цвет района — его оценка D (красный → жёлтый → зелёный), выбор района открывает его профиль.
...
Нужен **Python 3.11–3.14**. `app.py` запускает локальный Python API и отдаёт HTML/JavaScript-интерфейс из `web/` на том же порту. Node.js, npm, сборка фронтенда и запуск Streamlit не нужны. Скрипты запуска предусмотрены для Windows, Linux и macOS; Docker использует Python 3.12.
...
Первый запуск устанавливает Python-зависимости и может занять несколько минут. Основной интерфейс работает в браузере через локальный API; расчёты, оптимизатор и офлайн-советник работают без сети. Интернет нужен для установки пакетов и онлайн-советника. Остановить приложение — `Ctrl+C`.
...
| `app.py`, `ui/web_server.py`, `web/` | Python API и HTML/JavaScript-интерфейс: карта, каталог, результаты, лучшие планы, чат | `app.py`, `ui/web_server.py`, `web/` |
...
.venv/bin/python -B -m agent.self_check  # 12 проверок советника без сети и без ключа
```
...
```text
.
├── app.py                     # точка входа Python API и HTML-интерфейса
...
├── ui/                        # web_server.py — локальный HTTP API; прежние UI-компоненты
