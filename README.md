...
- **Карта пяти районов.** Полноэкранная карта Астаны (MapLibre): цвет района — его оценка D, переключатель «Было / Стало», 3D-режим, выбор района открывает его профиль. Без сети районы показываются карточками.
...
|---|---|
| *СКРИНШОТ: `docs/screenshots/01_city.png`* | Карта Астаны, Score «Качество жизни», вкладка «Город» |
| *СКРИНШОТ: `docs/screenshots/02_plan.png`* | Вкладка «Проекты» и лента «Ваш план» с бюджетом |
| *СКРИНШОТ: `docs/screenshots/03_result.png`* | Вкладка «Результат» и карта в режиме «Стало» |
| *СКРИНШОТ: `docs/screenshots/04_best_robust.png`* | Лучшие планы, устойчивость к событиям и сравнение альтернатив |
| *СКРИНШОТ: `docs/screenshots/05_advisor.png`* | Советник: разбор решения и «Как советник пришёл к выводу» |
...
Интерфейс ещё дорабатывается, поэтому ниже описаны шаги, а не расположение каждой кнопки.

1. **Открыть город.** Карта Астаны на весь экран, районы окрашены по оценке D. Слева — «Качество жизни · Astana» (база **52,56**, метка «ИСХОДНОЕ») и остаток бюджета. Клик по району открывает его профиль с показателями и кнопкой «Выбрать проекты».
...
| «Демо-сценарий» → «Рассчитать пример» (или вручную: M7, M8, M10 в Нуре, M12, M5 в Сарыарке) | **56,54** (+3,99), стоимость 95 |
...
flowchart LR
    ...
    subgraph AGENT["agent/ — ИИ-советник"]
        ...
        EVI["evidence<br/>ссылки и проверка скопированных чисел"]
    end
...
sequenceDiagram
    ...
    L-->>A: текст со ссылками или точно скопированными числами
    A->>A: evidence: сверить каждое число с ответами движка
...
.venv/bin/python -B -m agent.self_check  # 33 проверки советника без внешнего API
.venv/bin/python -B -m ui.web_check     # 13 проверок настоящего локального HTTP API
```
...
- Модель получает только инструменты движка: `baseline`, `validate`, `simulate`, `optimize`, `list_events`, `robustness`, `compare`. Каждое утверждение она обязана опирать на их реальные ответы.
- **Числа сверяются с движком.** Модель может вставить ссылку на поле ответа, например `{{e2:/results/0/score}}`, либо точно скопировать готовое число. `agent/evidence.py` проверяет ссылки и наличие числовых значений в ответах движка; отдельно проверяются роли Score и лимита бюджета. Это проверка фактов, а не доказательство смысла каждой фразы. Неизвестные числа, неверные ссылки и преобразования значений отклоняются; модели даётся одна попытка исправления.
...
| Вопрос | Наше решение |
|---|---|
| В ТЗ 5 районов, а сейчас в Астане 6: добавился Сарайшык, выделенный из Есильского района | Модель считает строго по ТЗ: 5 районов, их доли населения и исходные показатели не меняем, иначе перестанут совпадать эталоны 52.56 и 56.54. Территория Сарайшыка входит в «Есиль» из ТЗ и на карте показана в его составе. Шестой район — кандидат на следующую версию, когда появятся его реальные данные. |
